...
고유한 요소의 개수와 배열 크기의 절반 중 작을 값을 구하고 이중 더 작은 것을 찾아야함 

unordered_map 으로 첫번째 인수는 해당 숫자, 두번째 인수는 그 갯수를 저장한다. 
maxcase 는 해시맵의 크기, 즉 배열에 존재하는 고유한 정수의 갯수이다.
maxTypes 는 배열에서 선택할 수 있는 최대 정수의 갯수이다. 
...
int solution(vector<int> nums)
{
    ...
    int maxCase = hash.size();
    int maxTypes = nums.size() / 2;

    return min(maxCase, maxTypes);
}
